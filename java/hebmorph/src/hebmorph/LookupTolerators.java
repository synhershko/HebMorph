	...
	public static class TolerateEmKryiaYud implements ToleranceFunction
	{
		public Integer tolerate(char[] key, Reference<Integer> keyPos, String word, Reference<Float> score, char curChar)
		{
            ...
            if (curChar != 'י')
			{
				...
				if ((key[keyPos.ref] == 'י') && (key[keyPos.ref - 1] == 'י'))
				{
					score.ref *= 0.9f;
					keyPos.ref++;
					return 0;
			...
			if (word.charAt(word.length() - 1) == 'י')
			{
				...
				if ((key[keyPos.ref - 1] != 'י') || ((keyPos.ref + 1 == key.length) && (key.length <= 3)))
				{
					return null;
				}
			...
			else if (word.charAt(word.length() - 1) != 'ו')
			{
				score.ref *= 0.8f;
				return 1;
			}
	...
	*/
	public static class TolerateEmKryiaVav implements ToleranceFunction
	{
		public Integer tolerate(char[] key, Reference<Integer> keyPos, String word, Reference<Float> score, char curChar)
		{
            if ((curChar != 'ו') || // check current trie position
                    ...
                    (key[keyPos.ref] == 'י') || (key[keyPos.ref] == 'ה') || // Vav shouldn't be tolerated before a Yud or a Heh
                    (key[keyPos.ref] == 'ו' // Don't low-rank exact matches
)
                    )
                    return null;

			char prevChar = word.charAt(word.length() - 1);
            if ((key[keyPos.ref + 1] != 'ו') && (prevChar != 'ו') && // This case is handled by TolerateNonDoubledConsonantVav
	...
	public static class TolerateNonDoubledConsonantVav implements ToleranceFunction
	{
		public Integer tolerate(char[] key, Reference<Integer> keyPos, String word, Reference<Float> score, char curChar)
		{
            ...
            if ((curChar == 'ו') || (keyPos.ref == 0) || (keyPos.ref + 1 == key.length))
                return null;

            if ((key[keyPos.ref] == 'ו') && (word.charAt(word.length() - 1) == 'ו'))
			{
				keyPos.ref++;
				score.ref *= 0.8f;
				return 0;
